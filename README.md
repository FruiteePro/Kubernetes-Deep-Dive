...
| 阅读进度 | **读到正文末尾**才自动标记已读（一屏就能读完的短文章打开即算），侧栏显示 `已读 x/54` 进度条与 ✓，`r` 手动切换，存在浏览器 localStorage |
| 主题 | `t` 切换亮色 / 暗色，记住选择 |
...
- **阅读进度想重置**：进度存在浏览器 `localStorage`（key `k8s.read`），在页面控制台执行 `localStorage.removeItem('k8s.read')` 再刷新即可；清站点数据 / 换浏览器也会重置。
